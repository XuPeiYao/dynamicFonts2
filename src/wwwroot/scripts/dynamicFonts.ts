...
async function loadFonts(...fonts: string[]): Promise<void> {
  var allElement = document.body.querySelectorAll('*');
  if (fonts.length == 0) {
    fonts = [];
  ...
  for (var i = 0; i < allElement.length; i++) {
    var fontFamily = (
      window
        .getComputedStyle(allElement.item(i))
        .getPropertyValue('font-family') || ''
    )
      .split(',')
      .map(x => x.trim())
      .filter(x => fonts.indexOf(x) > -1);
    if (fontFamily.length == 0) continue;

    if (
      allElement[i].tagName == 'INPUT' ||
      allElement[i].tagName == 'TEXTAREA'
    ) {
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    for (let font in textTable) {
      ...
      loadSignleFont(font, textTable[font]).then(() => {
        ok[font] = true;
        let check = true;
...
var dynamicFontsHost = location.origin;
async function loadSignleFont(font: string, text: string): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    var xhr = new XMLHttpRequest();
    xhr.open('POST', dynamicFontsHost + '/api/font/' + font, true);
    ...
    xhr.onreadystatechange = () => {
      ...
      if (xhr.status == 200) {
        ...
        reader.onloadend = function() {
          ...
          style.setAttribute('dynamicFonts', '');
          style.innerHTML =
            '@font-face {' +
            '   font-family: "' +
            font +
